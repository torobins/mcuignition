...
| `RATEJUMP_ESCAPE_COUNT` | 3 | Consecutive rate-gate rejections tolerated before giving up on `lastPeriod` and rebuilding sync from scratch. See bug #8 below — without this the rate gate can lock out spark permanently, not just cost a couple of revolutions. |

Timebase is Timer5 at `/256` prescale (16 MHz → 16 µs/tick). The capture ISR keeps the leading edge of each pulse coil's signal and blanks the trailing-edge "twin" that a VR sensor produces per revolution, using `max(fixed floor, 1/2 of last period)`. Once synced, a rate-of-change gate also rejects any interval outside roughly 0.5×–2× the last measured period (catches a dropped or spurious extra edge immediately, rather than accepting it as a legitimate speed change) — with an escape hatch after `RATEJUMP_ESCAPE_COUNT` consecutive rejections so a bad reference period can't lock the gate shut forever. Spark and dwell are scheduled with Timer5 output compare (`OCR5A`/`OCR5B`), computed as an integer fraction of the last measured revolution period — no floating point in the ISR path.
...
Also aligned the dynamic blanking threshold with the rate gate's lower bound while fixing bug #8: blanking was `3/8 × lastPeriod`, the rate gate's lower bound is `1/2 × lastPeriod` — an edge landing in `[3/8, 1/2)` used to pass blanking only to hit the rate gate and drop sync, where a silent blank would have been the gentler outcome for the same signal. Raised blanking to `1/2` to match; margin against wrongly blanking a genuine speed increase is unchanged in practice (still needs >2× acceleration in one revolution either way).
